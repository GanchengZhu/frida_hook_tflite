...
Java.perform(
    function () {
        ...
        var native = Module.getExportByName("libc++_shared.so",
                        "memcpy");
        Interceptor.attach(native,{
            onEnter: function (args) {
                i += 1;
                console.log(i);
                console.log("Enter: " + new NativePointer(args[2]));
                console.log("Enter: " + new NativePointer(args[3]).toInt32());
...
function dump_memory(base,size,i) {
    ...
    var file_path = dir + "/dumpmemory_"+ i +".matdata";
...
# Then load uchar data by c++.
# Load .matdata file into uchar_data
